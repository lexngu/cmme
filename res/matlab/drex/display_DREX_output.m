...
p = pcolor(1+(0:sz(1)-1),1+(0:sz(2)-1),log10(mdl_out.context_beliefs));
...
subplot(5,1,4)
post_bd = post_DREX_beliefdynamics(mdl_out);
plot((1:size(feat_in,1)), post_bd(1:end-1,f),'-','color',col(f,:),'LineWidth',1);
hold all;
scatter((1:size(feat_in,1)), post_bd(1:end-1,f),20,col(f,:),'filled');
hold off;
xlim([0 size(feat_in,1)+1])
...
subplot(5,1,5)
post_cd = post_DREX_changedecision(mdl_out, 0.09);
plot((1:size(feat_in,1)), post_cd.changeprobability(1:end-1,f),'-','color',col(f,:),'LineWidth',1);
hold all;
scatter((1:size(feat_in,1)), post_cd.changeprobability(1:end-1,f),20,col(f,:),'filled');
hold off;
